...
public final class UnityPictureInPictureAdTest {
  ...
  @Captor private ArgumentCaptor<PictureInPictureAdRequest> adRequestCaptor;
  ...
  public void testLoad_onAdLoaded_invokesCallback() {
    unityPictureInPictureAd.load(AD_UNIT_ID, mockAdRequest);

    verify(mockAdWrapper).load(adRequestCaptor.capture(), adLoadCallbackCaptor.capture());
    assertThat(adRequestCaptor.getValue().getAdUnitId()).isEqualTo(AD_UNIT_ID);
