...
namespace GoogleMobileAds.iOS
{
    public class InterstitialAdPreloaderClient : IInterstitialAdPreloaderClient
    {
            ...
            Action<string> onAdsExhausted)
        {
            ...
            return Externs.GADUInterstitialAdPreloaderPreload(InterstitialAdPreloaderPtr, preloadId, preloadConfigRef);
        }

        public bool IsAdAvailable(string preloadId)
        {
            return Externs.GADUInterstitialAdPreloaderIsAdAvailable(InterstitialAdPreloaderPtr, preloadId);
        }
        ...
        public IInterstitialClient DequeueAd(string preloadId)
        {
            ...
            var interstitialAd = Externs.GADUInterstitialAdPreloaderGetPreloadedAd(InterstitialAdPreloaderPtr, preloadId,
                    interstitialAdClientPtr);
            if (interstitialAd == IntPtr.Zero) return null;
            interstitialAdClient.CreateInterstitialAdWithReference(interstitialAdClientPtr, interstitialAd);
            return interstitialAdClient;
        }

        public int GetNumAdsAvailable(string preloadId)
        {
            return Externs.GADUInterstitialAdPreloaderGetNumAdsAvailable(InterstitialAdPreloaderPtr, preloadId);
        }

        public PreloadConfiguration GetConfiguration(string preloadId)
        {
            var config = Externs.GADUInterstitialAdPreloaderGetConfiguration(InterstitialAdPreloaderPtr, preloadId);
...
#pragma warning restore CS0612
        ...
        private static void AdAvailableForPreloadIdCallback(IntPtr interstitialAdPreloaderClient, string preloadId, 
                                                IntPtr responseInfoClient)
        {
        ...
        private static void AdFailedToPreloadForPreloadIdCallback(IntPtr interstitialAdPreloaderClient, string preloadId,
                                                      IntPtr adErrorReference)
        {
            var client = IntPtrToInterstitialAdPreloaderClient(interstitialAdPreloaderClient);
