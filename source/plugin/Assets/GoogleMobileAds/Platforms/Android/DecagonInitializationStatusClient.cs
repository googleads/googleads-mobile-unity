...
{
    internal class DecagonInitializationStatusClient : IInitializationStatusClient
    {
        private AndroidJavaObject status;
        private AndroidJavaObject statusMap;

        public DecagonInitializationStatusClient(AndroidJavaObject status)
        {
            this.status = status;
        ...
        public AdapterStatus getAdapterStatusForClassName(string className)
        {
            ...
            AndroidJavaClass state = new AndroidJavaClass(DecagonUtils.AdapterStatusInitializationStateName);
            AndroidJavaObject completeEnum = state.GetStatic<AndroidJavaObject>("COMPLETE");
            AndroidJavaObject adapterLoadState = adapterStatus.Call<AndroidJavaObject>("getInitializationState");
            AdapterState adapterState =
                adapterLoadState.Call<bool>("equals", completeEnum) ? AdapterState.Ready : AdapterState.NotReady;
        ...
        private string[] getKeys()
        {
            ...
            AndroidJavaObject arrayObject = arrayClass.CallStatic<AndroidJavaObject>(
                  "newInstance", new AndroidJavaClass("java.lang.String"), map.Call<int>("size"));
