...
namespace GoogleMobileAds.Unity
{
    public class MobileAdsClient : IMobileAdsClient
    {
        ...
        public void OpenAdInspector(Action<AdInspectorErrorClientEventArgs> adInspectorClosedAction)
        {
            UnityEngine.Debug.Log("Dummy AdInspector");
            var client = new AdInspectorClient();
            client.OpenAdInspector((error) =>
            {
                if (error == null)
                {
                    adInspectorClosedAction(null);
                }
                else
                {
                    adInspectorClosedAction(new AdInspectorErrorClientEventArgs
                    {
                        AdErrorClient = error,
                    });
                }
