...
namespace GoogleMobileAds.Editor
{
    ...
    public class AdSizePropertyDrawer : PropertyDrawer
    {
        ...
        private readonly string[] adSizeEntries = new string[]
        {
            ...
            "Leaderboard (728x90)", "Smart Banner", "Anchored Adaptive Banner"
        };

        private readonly string[] adSizeValues = new string[]
        {
            BANNER, MEDIUM_RECTANGLE, IAB_BANNER,
            LEADERBOARD, SMART_BANNER, ANCHORED_ADAPTIVE_BANNER
        };
            ...
            if (ANCHORED_ADAPTIVE_BANNER.Equals(newAdSize))
            {
                ...
                if (!isFullWidth)
                {
                    int newWidth = EditorGUI.IntField(
                            GetNextFieldPositionRect(position), "Manual ad width (dp)", propWidth.intValue);
                    if (newWidth < 0)
                    {
                        newWidth = 0;
                    }
