...
namespace GoogleMobileAds.Api
{
    ...
    /// </summary>
    public class AppOpenAdPreloader
    {
        ...
        /// <summary>
        /// Raised when an ad fails to preload.
        /// </summary>
        public static event Action<string, AdError> OnAdFailedToPreload;

        /// <summary>
        /// Raised when a new ad is available for the given preload ID.
        /// </summary>
        public static event Action<string, ResponseInfo> OnAdAvailable;

        /// <summary>
        /// Raised when the last available ad is exhausted for the given preload ID.
        /// </summary>
        ...
        /// </param>
        /// <returns>
        /// False if preloading fails to start; otherwise, true.
        /// </returns>
        /// `onAdAvailable` and `onAdsExhausted` actions are optional.
        public static bool Preload(string preloadId, PreloadConfiguration preloadConfiguration)
        {
            _client.Preload(preloadId, preloadConfiguration);
            return false;
        }
        ...
        {
            return new AppOpenAd(_client.GetPreloadedAd(preloadId));
        }
        ...
        /// </returns>
        public static int GetNumberOfAdsAvailable(string preloadId)
        {
            return _client.GetNumberOfAdsAvailable(preloadId);
        }
        ...
        {
            _client.OnAdAvailable += (string preloadId, IResponseInfoClient responseInfo) =>
            {
                MobileAds.RaiseAction(() =>
                {
                    if (OnAdAvailable != null)
                    {
                        OnAdAvailable(preloadId, new ResponseInfo(responseInfo));
                    }
                });
            };

            _client.OnAdFailedToPreload += (string preloadId, IAdErrorClient error) =>
            {
                MobileAds.RaiseAction(() =>
                {
            ...
            _client.OnAdsExhausted += (string preloadId) =>
            {
                MobileAds.RaiseAction(() =>
                {
